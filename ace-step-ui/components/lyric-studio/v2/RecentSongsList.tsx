      ...
      {songs.slice(0, 8).map((rs) => {
        ...
        return (
          <button
            ...
            className="flex flex-col rounded-lg hover:bg-white/[0.04] transition-colors text-left group p-1.5 overflow-hidden"
          >
            {/* Cover art — larger for grid layout */}
            <div className="w-full aspect-square rounded-md overflow-hidden bg-zinc-800 relative mb-1.5">
              {rs.coverUrl ? (
              ...
              {/* Duration badge */}
              {rs.duration > 0 && (
                <span className="absolute bottom-1 right-1 text-[9px] text-white/80 bg-black/60 rounded px-1 py-0.5 font-mono">
                  {mins}:{secs}
                </span>
              )}
            </div>
            {/* Info */}
            <p className="text-[11px] font-medium text-zinc-200 truncate w-full leading-tight">
              {rs.recentSong.song_title || 'Untitled'}
            </p>
            <p className="text-[9px] text-zinc-500 truncate w-full leading-tight">
