...
import { X, Save, Loader2, FolderSearch, ChevronDown, ChevronRight, Zap, Music } from 'lucide-react';
import { lireekApi, AlbumPreset } from '../../../services/lyricStudioApi';
...
export const PresetSettingsModal: React.FC<PresetSettingsModalProps> = ({
  ...
  // Detect adapter type
  useEffect(() => {
    if (!form.adapter_path.trim() || !token) {
      setDetectedType(null);
      return;
    }
    let cancelled = false;
    generateApi.detectAdapterType(form.adapter_path.trim(), token)
      .then(result => { if (!cancelled) setDetectedType(result.type); })
      .catch(() => { if (!cancelled) setDetectedType('unknown'); });
    return () => { cancelled = true; };
  }, [form.adapter_path, token]);
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div
          className="w-full max-w-lg rounded-2xl bg-zinc-900 border border-white/10 shadow-2xl pointer-events-auto"
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      {form.adapter_path && (
                        <button
                          onClick={() => setForm(p => ({ ...p, adapter_path: '' }))}
                    ...
                    {form.adapter_path && adapterTag && (
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${adapterTagColor}`}>
                          {adapterTag}
                        </span>
                        <span className="text-[10px] text-zinc-500 truncate" title={form.adapter_path}>
                          {adapterFileName}
                        </span>
                      </div>
                    )}
                    ...
                    {groupsExpanded && (
                      <div className="space-y-2 pl-3 border-l-2 border-pink-500/20">
                        <EditableSlider
                          ...
                          tooltip="Self-Attention: internal temporal coherence — rhythmic patterns, melodic phrases, structural transitions."
                        />
                        <EditableSlider
                          label="Cross-Attn"
                          ...
                          tooltip="Cross-Attention: the bridge between your prompt and the output."
                        />
                        <EditableSlider
                          label="MLP"
                          value={form.mlp}
